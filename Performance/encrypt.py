...
m_ctxt = m_block.encrypt()

m1_block = heaan.Block(context, encrypted = False, data = [1] * num_slot)
m1_ctxt = m1_block.encrypt()

level_list = []
for i in range(12, 0, -1):
    
    print('***************** ciphertext m level : ', m_ctxt.level, '***********************')
    time_list = []
    for j in range(33):
        ...
        time_list.append(encryption_time)
        ...
        diff = abs(round(1 - res_ctxt[0].real, 12))
    ...
    print('first ciphertext : ',m_ctxt.level)

    m_ctxt = m_ctxt * m1_ctxt    
    print()
    print(' ──────────────────────────────────────────────────────────')
    print()
